...
class QueryBuilder:
    ...
    def guardar_consulta(self):
        """Guarda la consulta en un archivo JSON"""
        nombre_archivo = input("\n💾 Ingrese el nombre del archivo (sin extensión): ")
        if not nombre_archivo:
            nombre_archivo = "query_generada"
        
        archivo = f"{nombre_archivo}.json"
        
        try:
            with open(archivo, 'w', encoding='utf-8') as f:
                json.dump(self.query, f, indent=2, ensure_ascii=False)
            print(f"✅ Consulta guardada en: {archivo}")
            return archivo
    ...
    def ejecutar(self):
        ...
        # Guardar consulta
        archivo_guardado = self.guardar_consulta()
